...
var CompareOils = BaseView.extend({
  ...
  initialize: function () {
    ...
    this.tip = d3.tip()
      ...
      .html(function (d) {
        ...
        return utils.createTooltipHtml(d.y, d.type, values, utils.makeId(d.y), '', Oci.data.info[d.y]['Absolute Emissions Icons'], self.showCarbon, false, utils.getDataQuality(d.y).total, false);
      })
      .offset([0, 20])
      .direction('e');
  ...
  createChartData: function () {
    ...
    for (var i = 0; i < oils.length; i++) {
      ...
      var upstream = +modelData.oilValues[oils[i]].Upstream;
      var midstream = +modelData.oilValues[oils[i]].Midstream;
      var transport = 0;
      // var transport = +info[utils.getDatasetKey('transport')];
      var combustion = +modelData.oilValues[oils[i]].Downstream;
  ...
  updateAxes: function (animate) {
    ...
    var xMax = utils.getGlobalExtent('perBarrel', 'max');
  ...
  getXAxisTitle: function () {
    if (this.showCarbon) {
      ...
      return utils.getDatasetName(this.sortStep, 'perBarrel');
    }
  },
